...
fValidate.prototype.blank = function()
{
	...
	if ( this.isBlank() )
	{
		...
		this.throwError( [this.elem.getAttribute("caption")] );
	}
}
/*/>*/
/*< number numbers *******************************************************************/
fValidate.prototype.number = function( type, lb, ub )
{
	...
	if ( !fail )
	{
		switch( true )
		{
			case ( !isNaN(lb) && !isNaN(ub) ): fail = !Boolean( (lb <= num) && (num <= ub) ); break;
			case ( !isNaN(lb) ): fail = Boolean( num < lb ); break;
			case ( !isNaN(ub) ) : fail = Boolean( num > ub ); break;
		}
	}
	if ( fail )
	{
		this.throwError( [this.elem.getAttribute("caption"), lb, ub ] );
		return;
	}
	this.elemPass = true;
}
...
fValidate.prototype.numeric = function( len )
{
	...
	if ( !regex.test( this.elem.value ) )
	{
		if ( len == "*" )
		{
			...
			this.throwError( [this.elem.getAttribute("caption"), len], 1 );
		}
	}
}
/*/>*/
...
fValidate.prototype.length = function( len, maxLen )
{
	...
	if ( vlen > maxLen )
	{
		this.throwError( [this.elem.getAttribute("caption"), maxLen, vlen], 2 );
	}
}
/*/>*/
/*< alnum extended *******************************************************************/
...
/*/>*/
/*< chinapost international *******************************************************************/
fValidate.prototype.chinapost = function()
{
	var elem = this.elem;
	if ( this.typeMismatch( 'text' ) ) return;
	if ( !( /^[1-9]\d{5}$/.test( this.elem.value ) ) )
	{
		if ( this.isBlank() ){			
			this.throwError([this.elem.getAttribute("caption")], 1);
		}	else{
			this.throwError([this.elem.value]);
		}	
	}
}
/*/>*/
...
fValidate.prototype.ip = function( portMin, portMax )
{
	...
	else
	{
		...
		while ( part = parts[i++] )
		{
			if ( i == 5 ) // Check port
			{
				if ( parseInt(part) < portMin || parseInt(part) > portMax )
				{
					this.throwError( [this.elem.getAttribute("caption"), part, portMin, portMax], 1 );
				}
			}
...
/*/>*/
/*< multi ip web *******************************************************************/
fValidate.prototype.mip = function( )
{
	if ( this.typeMismatch( 'text' ) ) return;
	if ( !( /^\d{1,3}(\.\d{1,3}){3}(,\d{1,3}(\.\d{1,3}){3})*$/.test( this.elem.value ) ) )
	{
		this.throwError([this.elem.getAttribute("caption")]);
	}
}
/*/>*/
/*< multi ip with port web *******************************************************************/
fValidate.prototype.mipport = function( )
{
	if ( this.typeMismatch( 'text' ) ) return;
	if ( !( /^\d{1,3}(\.\d{1,3}){3}:\d{1,5}(,\d{1,3}(\.\d{1,3}){3}:\d{1,5})*$/.test( this.elem.value ) ) )
	{
		this.throwError([this.elem.getAttribute("caption")]);
	}
}
...
fValidate.prototype.date = function( formatStr, delim, code, specDate )
{
	...
	var error	= [this.elem.getAttribute("caption"), formatStr.replace( /\//g, delim )];
...
/*/>*/
/*< chinaIdCard *******************************************************************/
fValidate.prototype.chinaIdCard = function()
{
	var elem = this.elem;
	if ( this.typeMismatch( 'text' ) ) return;
	var idCartType = document.getElementById("idCardType");
	var type = 0;//默认为其它
	if(idCartType != null)
	{
		if(idCartType.value == "SF"){
			type = 1;
		} else if (idCartType.value == "JG"){
			type = 2;//军官证
		} else if (idCartType.value == "HZ"){
			type = 3;//护照
		} else if (idCartType.value == "HX"){
			type = 4;//回乡证
		} else if (idCartType.value == "QT"){
			type = 0;//其它
		}		
	} 
	if ( this.isBlank() )	
	...
	else
	{
		...
		{
			case 1:				
					if ( !( /^([1-9]\d{7}((0\d)|(1[0-2]))(([0|1|2]\d)|3[0-1])\d{3}|[1-9]\d{5}[1-9]\d{3}((0\d)|(1[0-2]))(([0|1|2]\d)|3[0-1])\d{3}([0-9]|X|x))$/.test( this.elem.value ) ) )
					{
						this.throwError([this.elem.value], 2);
					}
					break;	
			case 2:
					if ( !( /^[\u4e00-\u9fa5_a-zA-Z0-9-]+\u5b57\u7b2c[\u4e00-\u9fa5_a-zA-Z0-9-]+\u53f7$/.test( this.elem.value ) ) )					
					{
						this.throwError([this.elem.value], 3);
					}
					break;	
			case 4:
					if ( !( /^(H|h|M|m)\d{10}$/.test( this.elem.value ) ) )
					{
						this.throwError([this.elem.value], 4);
					}
					break;	
			case 3:
			case 0:
...
/*/>*/
/*< chinaMobilePhone 中国手机号码 *******************************************************************/
fValidate.prototype.chinaMobilePhone = function()
{
	var elem = this.elem;
	if ( this.typeMismatch( 'text' ) ) return;
	if ( !( /^(13|14|15|18)\d{9}$/.test( this.elem.value ) ) )
	{
		if ( this.isBlank() ){			
			this.throwError([this.elem.getAttribute("caption")], 1);
		}	else{
...
/*/>*/
/*< chinaPhoneFax 中国电话与传真号码 *******************************************************************/
fValidate.prototype.chinaPhoneFax = function()
{
	var elem = this.elem;
	if ( this.typeMismatch( 'text' ) ) return;
	if ( !( /^((\d{11})|(\d{7,8})|((\d{4}|\d{3})-(\d{7,8}))|((\d{4}|\d{3})-(\d{7,8})-(\d{4}|\d{3}|\d{2}|\d{1}))|((\d{7,8})-(\d{4}|\d{3}|\d{2}|\d{1})))$/.test( this.elem.value ) ) )
	{
		if ( this.isBlank() ){			
			this.throwError([this.elem.getAttribute("caption")], 1);
		}	else{
...
/*/>*/
/*< chinaOuCode 组织机构代码、工商营业执照、税务登记证、其它 *******************************************************************/
fValidate.prototype.chinaOuCode = function()
{
	var elem = this.elem;
	if ( this.typeMismatch( 'text' ) ) return;
	var ouCodeType = document.getElementById("ouCodeType");
	var type = 0;//默认为其它
	if(ouCodeType != null)
	{
		if(ouCodeType.value == "JJ"){
			type = 1;//组织机构代码
		} else if (ouCodeType.value == "GS"){
			type = 2;//工商营业执照
		} else if (ouCodeType.value == "SW"){
			type = 3;//税务登记号
		} else if (ouCodeType.value == "QT"){
			type = 0;//其它
		}		
	} 
	if ( this.isBlank() )	
	...
	else
	{
		...
		{
			case 1:				
					if ( !( /^(([0-9A-Z]{8}-[0-9A-Z])|[0-9A-Z]{9})$/.test( this.elem.value ) ) )
					{
						this.throwError([this.elem.value], 2);
					}
					break;	
			case 2:
					if ( !( /^([0-9]{15})$/.test( this.elem.value ) ) ) 
					{
						this.throwError([this.elem.value], 3);
					}
					break;	
			case 3://分为机构的15位(六位行政区划代码和九位组织机构代码)和个人的20位
					if ( !( /^(([0-9]{6}[0-9A-Z]{9})|[0-9x]{20})$/.test( this.elem.value ) ) )
					{
						this.throwError([this.elem.value], 4);
					}
					break;	
...
/*/>*/
/*< chinacity 中国城市，目前只支持地区级城市***************************************************************/
fValidate.prototype.chinacity = function()
{
	var elem = this.elem;
	if ( this.typeMismatch( 'text' ) ) return;
	var province = document.getElementById("chinaProvince");
	var type = 0;//其它
	if(province != null)
	{
		if(province.value == "北京" || province.value == "北京市" ){
			type = 1;
		} else if(province.value == "上海" || province.value == "上海市" ){
			type = 2;
		} else if(province.value == "天津" || province.value == "天津市" ){
			type = 3;
		} else if(province.value == "重庆" || province.value == "重庆市" ){
			type = 4;	
		} else if(province.value == "江苏" || province.value == "江苏省" ){
			type = 5;	
		} else if(province.value == "新疆" || province.value == "新疆省" || province.value == "新疆维吾尔自治区"){
			type = 6;	
		} else if(province.value == "福建" || province.value == "福建省" ){
			type = 7;			
		} else if(province.value == "河北" || province.value == "河北省" ){
			type = 8;			
		} else if(province.value == "湖南" || province.value == "湖南省" ){
			type = 9;														
		} else {
			type = 0;//其它，暂不支持
		}		
	...
	else
	{
		switch(type)
		{
			...
			case 4:
				...
				break;	
			case 5:
				if ( !( /^(南京|徐州|连云港|淮安|宿迁|盐城|扬州|泰州|南通|镇江|常州|无锡|苏州|南京市|徐州市|连云港市|淮安市|宿迁市|盐城市|扬州市|泰州市|南通市|镇江市|常州市|无锡市|苏州市)$/.test( this.elem.value ) ) ) 
				{
						this.throwError([this.elem.value], 2);
				}	
				break;
